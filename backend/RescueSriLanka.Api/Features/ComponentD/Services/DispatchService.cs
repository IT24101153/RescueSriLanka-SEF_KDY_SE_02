...
namespace RescueSriLanka.Api.Features.ComponentD.Services
{
    ...
    public class DispatchService : IDispatchService
    {
        ...
        private static bool TryGetApprovedValidation(AgentWorkflow workflow, Assignment assignment, int planVersion, out string error)
        {
            error = "Safety validation is invalid or stale.";
            if (workflow.Status != WorkflowStatus.AwaitingApproval) return false;
            try
            {
            ...
            if (dto.NewStatus is DispatchStatus.Resolved or DispatchStatus.Cancelled)
            {
                ...
                if (dto.NewStatus == DispatchStatus.Resolved)
                {
                    var matchingWorkflows = await _db.AgentWorkflows
                        .Where(w => w.Status == WorkflowStatus.Executing && w.FinalOutcomeJson != null)
                        .ToListAsync();
